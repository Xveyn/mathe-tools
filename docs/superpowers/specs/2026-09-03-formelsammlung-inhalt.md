...
Im Ordner liegt zusätzlich `MA2_ueb_DGL-4.pdf`; es ist byte-identisch mit `MA2_ueb_DGL-3.pdf` (gleiche MD5-Summe) und wird deshalb nicht getrennt geführt.
...
1. **Lineare Algebra ist das Kapitel.** 60 von 143 Aufgaben, 52 von 130
   ...
   GF(3), GF(5), GF(11), dazu „modulo 7" ohne die GF-Schreibweise — nicht nur
   über ℝ und ℂ. Eine Formelsammlung, die
...
6. **Die DGL-Aufgaben rechnen in drei Schreibweisen, das Skript nur in einer.**
   Das Skript schreibt durchgehend `y'' + αy' + βy = γ(x)` mit `D = (α/2)² − β`.
   Auf dem DGL-Blatt stehen daneben:
   - **Leibniz-Notation** in den Aufgaben 5, 6 und 7: `d²y/dt² + (1/τ)·dy/dt + ω₀²y = …`
   - **Punkt-Notation** in den Aufgaben 9 und 12: `ÿ + ω₀²y = c·sin(ω₀t)`
   - Aufgabe 4 ist keine DGL zweiter Ordnung, sondern eine **erster** Ordnung in
...
- **C** — wird zum Lösen keiner Aufgabe gebraucht; steht der Vollständigkeit
  halber da. Eine Aufgabe darf den Begriff streifen, ohne dass man den Eintrag
  zum Rechnen braucht.
...
| 1.2 | Summierte Sehnentrapezformel `(b−a)/n · (½f(a) + f(x₁) + … + ½f(b))` | 13, 15 | — | C |
...
| 1.9 | **Grundintegrale-Tabelle**: `xˢ` (s ≠ −1), `1/x`, `eᵅˣ`, `sin`, `cos`, `1/(1+x²)`, `1/√x`, `cot` | 24, 35 | 5, 9, 17, 19 | A |
...
| 1.15 | Der Trick: zweimal partiell integrieren und nach dem Integral auflösen — im Skript an `∫sin²`, in Aufgabe 15 an `∫e⁻ˣcos x` | 29–30 | 15, 19a | A |
...
| 1.22 | **Polynomdivision zuerst**: nur eine echt gebrochen rationale Funktion (`grad p < grad q`) lässt sich zerlegen | 35, 37 | 22a | A |
| 1.23 | Jede stetige und jede monotone Funktion ist integrierbar | 17 | — | C |
...
| 2.9 | **Ansatztabelle zweiter Ordnung** samt Resonanz: Polynom (`β≠0` / `β=0,α≠0` / `α=β=0`), Polynom·e^{ux} (`u≠λ₁,λ₂` / `u=λ₁≠λ₂` / `u=λ₁=λ₂`), `a·cos+b·sin` (`iω≠λ` / `iω=λ`) | 50 | 2e–2i, 8, 10–12, 15 | A |
| 2.10 | Das Vier-Schritt-Verfahren (homogen lösen, spezielle Lösung, Summe, Anfangswerte einsetzen) | 53 | alle | A |
| 2.11 | Superposition: Summe mehrerer Störglieder → jeder Summand einzeln, Ergebnisse addieren | — (nur Aufgaben) | 10, 11 | A |
| 2.12 | **Die drei Schreibweisen nebeneinander**: Strich (Skript), Leibniz `d²y/dt²` (Aufg. 5–7), Punkt `ÿ` (Aufg. 9, 12); dazu `ω₀`, `τ` und `ω = √(ω₀² − 1/(4τ²))` (nur Aufg. 5) | — (nur Aufgaben) | 5–7, 9, 12 | A |
| 2.13 | Serien-Schwingkreis: `U′ = LI″ + RI′ + I/C`; die Normalform `I″ + (R/L)I′ + I/(LC) = 0` steht nur in Aufgabe 3 | 46 | 3, 5, 6 | B |
| 2.14 | RC-Kreis: `U′_C = −U_C/(RC) + U₀/(RC)` mit der Lösung `U₀(1 − e^{−t/(RC)})` | 43–44 | 4 | B |
...
| 3.3 | Restklassen `x ≡ y mod n`, Standardrepräsentant aus `{0,…,n−1}`; Rechnen mit beliebigen Repräsentanten | 56, 60 | 2, 5, 6, 7 | A |
...
| 3.6 | Invertierbar ⟺ teilerfremd; das Skript führt die **Inversentabelle nur für GF(7)** (S. 60) und zu GF(5) ein einzelnes Beispiel (S. 59). Die Tabellen für **GF(3), GF(5) und GF(11) rechnet die Sammlung selbst** und kennzeichnet sie so | 59–60 | 6, 11, 25e, 44 | A |
...
| 3.12 | **Abbildungsmatrix**: die Bilder der Einheitsvektoren in die Spalten | 66 | 13, 16–19, 21 | A |
...
| 3.21a | **Gauß-Verfahren**: Zeilenstufenform, freie Variablen, die Lösungsmenge als lineare Hülle — das Werkzeug hinter 3.14, 3.19, 3.34 und 3.45 | 68, 73, 74, 87–88 | fast alle | A |
| 3.21b | Matrizenmultiplikation und Transponierte als Rechenregel; `f(xᵀ) = xᵀAᵀ` | 63, 66 | 14, 21 | B |
| 3.21c | **Dimensionsformel** `dim ker(f) + dim im(f) = n` | nicht im Skript | 23 | A |
| 3.21d | Multiplikation mit `w = a+bi` als reelle 2×2-Abbildung der Ebene | nicht im Skript | 15 | B |
...
| # | Eintrag | Skript | Aufgaben | Prio |
|---|---|---|---|---|
| 3.35 | Orthogonal `A⁻¹ = Aᵀ`; unitär `A⁻¹ = Āᵀ`; Prüfung über `AAᵀ = E` ohne Invertieren | 82, 85 | 34–36, 39, 40 | A |
...
| 3.40 | **Im Raum**: Drehung um die z-Achse, Spiegelung an der x,z-Ebene, Drehspiegelung | 84 | 18, 35, 53 | A |
| 3.40a | Drehung um die **y**- und die **x**-Achse — im Skript steht nur die z-Achse, Bem. 3.9.4 nennt die Beispiele ausdrücklich einen Spezialfall | nicht im Skript | 35b, 60b | A |
| 3.41 | `det` einer reellen orthogonalen Matrix ist `±1` | — (Folgerung) | 40j, 40k | B |
| 3.42 | Beispiel unitär: `(e^{iα})` als 1×1-Drehung; DFT-Matrix in Dimension 4 | 84–85 | 39 | B |
...
| 3.47 | Dreiecksmatrix: Eigenwerte sind die Diagonaleinträge | 89 | 41d | A |
...
| 3.58a | **Abbildungsmatrix aus einer ONB zurückrechnen**: `A = T·S·Tᵀ` — die Richtung, die man für eine Spiegelung an einer Ebene braucht | 96–97 | 53d | A |
| 3.59 | **Spektralzerlegung** `A = λ₁v₁v₁ᵀ + … + λₙvₙvₙᵀ` | 97 | 52d | A |
| 3.60 | **SVD** `A = USVᵀ`, Singulärwerte absteigend, `r = rg(A)`, `A = σ₁u₁v₁ᵀ + …` | 99 | 57 | A |
| 3.61 | **Pseudoinverse** `A⁺ = VS⁺Uᵀ`; links- oder rechtsinvers je nach Rang; `x = A⁺b` löst das Ausgleichsproblem | 100–101 | 57c–57e | A |
...
| # | Eintrag | Skript | Aufgaben | Prio |
|---|---|---|---|---|
| 3.64 | Einbettung `(x,y) ↦ [x,y,1]`; letzte Zeile der Matrix ist `(0 … 0 1)` | 103–104 | 58, 59, 60 | A |
...
| 3.67 | **Drehung um einen Punkt** = Verschiebung, Drehung, Rückverschiebung (Matrizen multiplizieren). Das Skript zeigt auf S. 104 nur Verschiebung **und danach** Drehung um den Ursprung, nicht die Konjugation | nicht im Skript | 58c, 59c, 60b | A |
...
| # | Eintrag | Skript | Aufgaben | Prio |
|---|---|---|---|---|
| 4.1 | Skalarfeld gegen Vektorfeld | 107 | 4, 18 | B |
| 4.1a | **Höhenlinien** `f(x,y) = c` und die beiden Scharen von **Schnittkurven** `z = f(x,c)` und `z = f(c,y)` | 107–108 | 1, 2, 7, 8 | A |
| 4.1b | Maximaler Definitionsbereich `D_f` einer mehrvariabligen Funktion | 107 | 3 | B |
...
| 4.6 | **Gradient** `grad f = (∂f/∂x₁, …, ∂f/∂xₙ)`, auch `∇f`; zeigt in Richtung des steilsten Anstiegs | 114 | 12, 20, 22, 27 | A |
| 4.7 | Näherung `Δz ≈ grad f(a)·Δx` | 115 | 38, 39, 42, 43 | A |
| 4.8 | Gradientenabstieg `a₁ = a₀ − γ·grad f(a₀)` | 115 | — | C |
| 4.9 | **Linearisierung / Tangentialebene** `z = f(a) + grad f(a)·(x−a)`, dazu die Koordinatendarstellung | 116–117 | 21, 22 | A |
| 4.10 | **Jacobi-Matrix** (Gradienten in die Zeilen); Linearisierung eines Vektorfeldes `z = f(a) + J_f(a)(x−a)` | 117–118 | — | C |
...
| 4.17 | Für `n = 2` das Schulkriterium über `det H` und `f_xx`. **Keine Aufgabe verlangt es** — sie sind alle mit dem Eigenwertkriterium des Skripts lösbar; es ist der schnellere Weg, nicht der geforderte | nicht im Skript | 29–31, 40 | B |
...
| 4.24 | **Implizite Funktionen**: Voraussetzung `∂F/∂y ≠ 0`, Ableitung `f′ = −(∂F/∂x)/(∂F/∂y)`, Tangentengleichung | 129–130 | 44, 45, 46 | A |
...
**Nur die kleinsten Quadrate stehen hier.** Alles andere, was das Skript nicht
hat, ist in seinem Kapitel eingeordnet und dort in der Spalte „Skript" mit
`nicht im Skript` gekennzeichnet — 1.22 ist es nicht, wohl aber 2.11, 2.12,
3.21c, 3.21d, 3.34, 3.40a, 3.41, 3.52, 3.67, 3.68, 4.17 und 4.23. Eine frühere
Fassung führte drei davon (Gauß-Inverse, Skalierung mit Fixpunkt,
Physik-Schreibweise) **zusätzlich** hier auf und zählte sie damit doppelt.

Die kleinsten Quadrate bleiben als eigener Block stehen, weil sie kein
Kapitel haben: sie gehören thematisch zu den Extrema, sind aber ein
geschlossenes Verfahren mit sechs eigenen Aufgaben.
...
## Woher die Korrekturen stammen

Diese Aufnahme wurde am 2026-09-03 von einer unabhängigen Prüfung gegen das
Skript und alle vier Übungsblätter gehalten. Sie hat vier sachliche Fehler
gefunden — die Drehungen um die y- und x-Achse stehen **nicht** im Skript, die
Inversentabelle gibt es dort nur für GF(7), die Drehung um einen Punkt ist nur
ein Aufgabentipp, und die Beschreibung der DGL-Schreibweisen war falsch —, dazu
dreizehn falsche Seitenzahlen, eine Dreifachzählung und rund zehn Lücken. Alles
ist eingearbeitet; die Belege stehen in den betroffenen Zeilen.

**Was ungeprüft blieb:** die genaue Zeichensetzung einzelner Formeln. Die
Textextraktion verstümmelt sie, und geprüft wurden im Klartext nur die Formeln,
die einen Befund tragen. Für das Abschreiben gilt der Hinweis oben unverändert.

## Zahlen zum Umfang

| Kapitel | Einträge | davon A | davon B | davon C |
|---|---|---|---|---|
| 1 Integralrechnung | 23 | 17 | 3 | 3 |
| 2 Differentialgleichungen | 14 | 11 | 3 | 0 |
| 3 Lineare Algebra | 75 | 56 | 16 | 3 |
| 4 Mehrere Variablen | 28 | 17 | 6 | 5 |
| Z kleinste Quadrate | 4 | 3 | 1 | 0 |
| **Summe** | **144** | **104** | **29** | **11** |

144 Einträge, davon 104 mit Priorität A. Das ist zu viel für eine Seite und zu
